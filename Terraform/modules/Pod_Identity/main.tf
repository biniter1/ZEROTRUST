...
# ── ESO role (External Secrets Operator) ─────────────────────
resource "aws_iam_role" "eso_role" {
  name = "eks-pod-external-secrets-role"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid    = "EKSPodIdentity"
...
resource "aws_iam_role_policy" "eso_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "arn:aws:secretsmanager:${var.aws_region}:${var.production_account_id}:secret:prod/*"
    }]
  })
}
